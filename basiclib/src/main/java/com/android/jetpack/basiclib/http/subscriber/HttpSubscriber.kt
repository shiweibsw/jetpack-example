package com.android.jetpack.basiclib.http.subscriber
...
class HttpSubscriber<T>(listener: OnResultCallBack<T>, lifecycle: Lifecycle) : Observer<T>,
    LifecycleObserver {
            ...
            e.exceptions.forEach {
                if (it is SocketTimeoutException) {
                    mOnResultListener?.onError(
                        ApiException.Code_TimeOut,
                        ApiException.SOCKET_TIMEOUT_EXCEPTION
                    );
                } else if (it is ConnectException) {
                    mOnResultListener?.onError(
                        ApiException.Code_UnConnected,
                        ApiException.CONNECT_EXCEPTION
                    );
                    ...
                    mOnResultListener?.onError(
                        ApiException.Code_MalformedJson,
                        ApiException.MALFORMED_JSON_EXCEPTION
                    );
                }
            }
